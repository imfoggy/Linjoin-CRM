...
class Customer extends Model{
	 ...
	 * @param  integer           $public  [是否是客户池数据]
	 * @return [type]                    [description]
	 */
	public static function getCustomers($where = [], $limit = 10){
		$result = self::with('getCreateUser,getUser,getCountry')->where($where)->order('id desc')->paginate($limit)->each(function($item, $key){
			$item['diffDays'] = '-';
			if($item['customer_log_number'] <= 0 && $item['is_public'] == 0){
				//计算还有几天就会移动到客户池
	 ...
	 * @return [type]                [description]
	 */
	public function getCustomerInfoById($id = 0){
		$result = self::get($id);
		$result->customerLogs;
		foreach($result['customerLogs'] as $key=>&$value){
	...
	public function customerLogs(){
		return $this->hasMany('CustomerLog', 'customer_id');
	}
	...
	public function contacts(){
		return $this->hasMany('CustomerContact', 'customer_id');
	}
	...
	public function userRecorder(){
		return $this->hasMany('CustomerUserRecorder', 'customer_id');
	}
	...
	public function files(){
		return $this->hasMany('Files', 'out_id')->where('from', '=', 'Customer');
	}
